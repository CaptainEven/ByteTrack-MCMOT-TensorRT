...
namespace byte_kalman
{
	...
	KAL_DATA KalmanFilter::initiate(const DETECT_BOX& measurement)
	{// xyah
	...
	Eigen::Matrix<float, 1, -1>
		KalmanFilter::gating_distance(
			const KAL_MEAN& mean,
			const KAL_COVA& covariance,
			const std::vector<DETECT_BOX>& measurements,
			bool only_position)
	{
		...
		DETECT_BOXES d(measurements.size(), 4);
		int pos = 0;
		for (DETECT_BOX box : measurements)
		{
			d.row(pos++) = box - mean1;
		}
