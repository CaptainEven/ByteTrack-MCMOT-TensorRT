...
vector<Track*> BYTETracker::joinTracks(vector<Track*>& tlista, vector<Track>& tlistb)
{
	map<int, int> exists;
	vector<Track*> res;
...
vector<Track> BYTETracker::subTracks(vector<Track> &tlista, vector<Track> &tlistb)
{
	map<int, Track> tracks;
	for (int i = 0; i < tlista.size(); i++)
	{
		tracks.insert(pair<int, Track>(tlista[i].track_id, tlista[i]));
	}
	for (int i = 0; i < tlistb.size(); i++)
	{
		int tid = tlistb[i].track_id;
...
void BYTETracker::removeDuplicateTracks(vector<Track>& res_a, 
	vector<Track>& res_b,
	...
	vector<Track>& tracks_b)
{
	...
	{
		int timep = tracks_a[pairs[i].first].frame_id - tracks_a[pairs[i].first].start_frame;
		int timeq = tracks_b[pairs[i].second].frame_id - tracks_b[pairs[i].second].start_frame;
		if (timep > timeq)
...
void BYTETracker::linearAssignment(vector<vector<float>>& cost_matrix,
...
vector<vector<float> > BYTETracker::iouDistance(vector<Track*> &atracks, vector<Track> &btracks, int &dist_size, int &dist_size_size)
{
...
vector<vector<float> > BYTETracker::iouDistance(vector<Track> &atracks, vector<Track> &btracks)
{
	vector<vector<float> > atlbrs, btlbrs;
	for (int i = 0; i < atracks.size(); i++)
	{
...
Scalar BYTETracker::getColor(int idx)
{
	idx += 3;
